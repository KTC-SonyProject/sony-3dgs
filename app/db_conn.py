...
class DatabaseHandler:
    def __init__(self, settings: dict, use_connection_kwargs: bool = False):
        """
        データベース接続情報を初期化するクラス
        ...
        self.use_connection_kwargs = use_connection_kwargs
    ...
    def _init_postgres(self):
        ...
        if self.use_connection_kwargs:
            kwargs = {"autocommit": True, "prepare_threshold": 0}
            self.pool = ConnectionPool(conninfo=db_uri, max_size=20, kwargs=kwargs, open=True)
            logging.info("PostgreSQL connection pool is initialized with connection kwargs.")
        else:
